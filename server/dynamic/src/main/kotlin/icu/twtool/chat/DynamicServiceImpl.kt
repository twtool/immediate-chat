                ...
                when (message) {
                    is PublishDynamicEvent -> {
                        ...
                        val success = redis.lock(lockKey) {
                            ...
                            val models = mutableListOf<Timeline>(
//                                Timeline(message.uid, message.uid, message.id, message.time)
                    ...
                    is DeleteDynamicEvent -> {
                        // 加分布式锁
                        val lockKey = "timeline:handle:dynamic:${message.id}"
                        val success = redis.lock(lockKey) {
    ...
    override suspend fun publish(param: PublishDynamicParam): Res<Unit> {
        ...
        db.transaction {
            ...
            // 推送到 MQ 处理好友的时间线, 自己的时间线直接添加避免回显延时
            assertTrue(TimelineDao.insert(Timeline(loggedUID, loggedUID, dynamicId, time)))
    ...
    override suspend fun delete(dynamicId: String): Res<Unit> {
        val id = assertNotNull(dynamicId.toLongOrNull(), CommonStatus.ParamErr)
        val loggedUID = loggedUID()

        db.transaction {
            assertTrue(DynamicDao.deleteById(loggedUID, id))
            // TODO: 删除和动态相关的数据（eg：COS 中的附件）
//            assertTrue(DynamicAttachmentDao.deleteByDynamicId(id))
//            assertTrue(DynamicLikeDao.deleteByDynamicId(id))
//            assertTrue(DynamicCommentDao.deleteByDynamicId(id))

            // 推送到 MQ 处理好友和自己的时间线
            timelineHandleProducer.send(
                buildMessage(
                    DYNAMIC_TIMELINE_HANDLE_TOPIC,
                    JSON.encodeToString<TimelineEvent>(DeleteDynamicEvent(id)).toByteArray(),
                    messageGroup = "$loggedUID",
                    keys = arrayOf(loggedUID.toString())
                )
            )
